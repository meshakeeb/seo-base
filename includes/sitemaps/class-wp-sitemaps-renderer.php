...
class WP_Sitemaps_Renderer {
				...
				if ( 'loc' === $name ) {
					...
					_doing_it_wrong(
						...
						sprintf(
							...
							__( 'Fields other than %s are not currently supported for the sitemap index.', 'nhg-seo' ), // phpcs:ignore
							implode( ',', [ 'loc', 'lastmod' ] )
						),
						'5.5.0'
					);
	...
	public function get_sitemap_xml( $url_list ) {
		...
		$geo_location = apply_filters( 'nhg_seo_sitemap_image_geo', 'Oslo, Norway' );
		...
		foreach ( $url_list as $url_item ) {
			...
			foreach ( $url_item as $name => $value ) {
				if ( 'loc' === $name ) {
					...
					foreach ( $value as $attachment_url ) {
						$image = $url->addChild( 'image:image', null, 'http://www.google.com/schemas/sitemap-image/1.1' );
						$image->addChild( 'image:loc', $attachment_url, 'http://www.google.com/schemas/sitemap-image/1.1' );
						$image->addChild( 'image:geo_location', $geo_location, 'http://www.google.com/schemas/sitemap-image/1.1' );
					}
				} else {
		...
		if ( ! class_exists( 'SimpleXMLElement' ) ) {
			...
			wp_die(
				sprintf(
					...
					esc_xml( __( 'Could not generate XML sitemap due to missing %s extension', 'nhg-seo' ) ), // phpcs:ignore
					'SimpleXML'
				),
				esc_xml( __( 'WordPress &rsaquo; Error', 'nhg-seo' ) ), // phpcs:ignore
				[
